...
<samp>**27** games · **1,551** config options · **67** API routes · **472** tests · **146** security checks</samp>
...
| Script | Checks |
|:--|:--|
| `npm test` | 472 tests over the config renderer, path guard, auth, pagination, API key scopes, server lifecycle rules, and **database integrity, end-to-end installer round-trips, and multi-write atomicity against a real PostgreSQL** *(see below)* |
...
| `npm run verify:security` | 146 regression checks pinning the security audit fixes: path containment, backup-name allowlisting, SQL identifier quoting, JWT policy, security headers, and a sweep for leaked exception messages |
