...
# ET:Legacy loads configs relative to the mod directory (etmain/),
# so "exec server.cfg" looks for etmain/server.cfg.
mkdir -p etmain
...
// Default map rotation — server MUST have a map to stay alive
...
echo "ET:Legacy installed successfully"`,
    startCommand: `cd {{INSTALL_PATH}} && ./etlded +set dedicated 2 +set net_port {{PORT}} +set fs_basepath "{{INSTALL_PATH}}" +set fs_homepath "{{INSTALL_PATH}}" +set fs_game etmain +exec server.cfg`,
    stopCommand: null,
