...
export const API_DOCS: ApiDocGroup[] = [
  {
    ...
    icon: "🔐",
    note: "Session routes use the gsm_token cookie. Login is username + password (optional 2FA). The CD-key panel gate was removed in Stage 42 — the only key left is the operator master key, which guards fresh installs.",
    endpoints: [
      { method: "POST", path: "/api/auth/login", description: "Sign in (username + password, optional 2FA code).", auth: "public (throttled)" },
      { method: "POST", path: "/api/auth/register", description: "Create an account (age-gated).", auth: "public (throttled)" },
      ...
      { method: "POST", path: "/api/auth/reset-password", description: "Consume a reset token.", auth: "public (throttled)" },
      { method: "POST", path: "/api/install", description: "Claim a fresh panel. Requires the operator master key when one is configured.", auth: "public (first-run)" },
      { method: "GET", path: "/api/settings/master-key", description: "Master key status (stored/env booleans — never the key).", auth: "panel.settings" },
      { method: "POST", path: "/api/settings/master-key", description: "Rotate the master key (returns plaintext once).", auth: "panel.settings" },
      { method: "DELETE", path: "/api/settings/master-key", description: "Revoke the stored master key.", auth: "panel.settings" },
    ],
  },
  {
    title: "Servers",
