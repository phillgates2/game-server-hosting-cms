...
export default function PublicChatWidget({
  ...
  const config: ChatConfig = { ...DEFAULT_CONFIG, ...configOverride };
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Clamp to viewport
      const maxX = window.innerWidth - (config.width || 360);
  ...
  }, [isDragging, dragOffset, config.width]);
  ...
  const widgetWidth = Math.max(280, Math.min(config.width, 600));
  const widgetHeight = Math.max(200, Math.min(config.height, 800));
  ...
  return (
    <div
      ...
      style={{
        ...positionStyle,
        width: minimized ? widgetWidth : widgetWidth,
      }}
      className="select-none"
    >
      <div
        className="gaming-surface rounded-xl overflow-hidden flex flex-col shadow-2xl"
        style={{ maxHeight: minimized ? "auto" : widgetHeight }}
      >
        ...
        <button
          ...
          onClick={(e) => {
            ...
            if (!isDragging) setMinimized(!minimized);
            e.preventDefault();
          }}
          className="flex items-center justify-between px-4 py-3 bg-bg-secondary/60 border-b border-border/50 hover:bg-bg-hover/50 transition-colors cursor-grab active:cursor-grabbing w-full text-left"
        >
                ...
                <div className="flex items-center justify-center h-full text-text-muted text-sm py-8">
                  <div className="text-center">
                    ...
                    <p className="text-xs mt-1">
                      {user ? "Be the first to say hello!" : "Login to join the conversation!"}
                    </p>
                  </div>
                </div>
              ) : (
