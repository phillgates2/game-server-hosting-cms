...
const POLL_INTERVAL = 2500; // 2.5 seconds for real-time feel

export default function SandboxChat({ user }: { user: AuthUser }) {
  ...
  const fetchMessages = useCallback(async (initial = false) => {
    try {
      ...
      const limit = initial ? 50 : 50;
      const res = await fetch(`/api/forum/chat?limit=${limit}${afterParam}`);
      if (!res.ok) return;
    ...
    <div className="gaming-surface rounded-xl overflow-hidden flex flex-col">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm">Sandbox Chat</span>
                  ...
                  >
                    ...
                    {(isOwn || isMod) && (
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 transition-opacity text-text-muted hover:text-danger text-xs px-1 flex-shrink-0 self-start mt-1"
