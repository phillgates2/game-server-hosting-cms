        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
          ...
          <nav className="flex items-center gap-1">
            {([["home","Home"],["blog","Blog"],["changelog","Changelog"]] as const).map(([k,l]) => (
              <button key={k} onClick={() => { setTab(k); setSelectedPost(null); }}
                className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${tab === k ? "text-accent bg-accent/10" : "text-text-secondary hover:text-text-primary hover:bg-bg-hover"}`}
              >{l}</button>
            ))}
            <button onClick={onLoginClick}
              className="ml-3 px-4 py-2 bg-accent hover:bg-accent-hover text-white text-sm font-medium rounded-lg transition-colors"
            >
              Login
            </button>
          </nav>
      ...
      <main className="flex-1 max-w-6xl mx-auto px-4 sm:px-6 py-8 w-full">
          ...
          <div className="animate-fade-in space-y-12">
            <section className="text-center py-16">
              <h1 className="text-4xl sm:text-5xl font-bold mb-4">
                <span className="bg-gradient-to-r from-accent to-purple bg-clip-text text-transparent">Game Server Hosting</span>
              </h1>
              <p className="text-text-secondary text-lg max-w-2xl mx-auto mb-8">
                High-performance game servers with a modern control panel. Multi-node infrastructure, real-time monitoring, and one-click deploys.
              </p>
              <div className="flex gap-4 justify-center">
                <button onClick={onLoginClick} className="px-6 py-3 bg-accent hover:bg-accent-hover text-white rounded-lg font-medium transition-colors">
                  Control Panel →
                </button>
                <button onClick={() => setTab("blog")} className="px-6 py-3 bg-bg-card border border-border hover:border-accent/30 rounded-lg font-medium transition-colors">
                  Read Blog
                </button>
              </div>
            </section>
            ...
            {loaded && blogs.length === 0 && changelogs.length === 0 && (
              <section className="text-center py-12 bg-bg-card border border-border rounded-xl">
                ...
                <p className="text-text-secondary text-sm">
                  Log in and create blog posts or changelogs from the CMS section.
                </p>
              </section>
            )}
          </div>
        ...
        {tab === "blog" && !selectedPost && (
          <div className="animate-fade-in space-y-6">
            ...
            {blogs.length === 0 && loaded ? (
              <Empty text="No blog posts yet." />
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {blogs.map((p) => <PostCard key={p.id} post={p} onClick={() => openPost(p)} fmt={fmt} />)}
              </div>
            )}
          </div>
        )}
        ...
        {tab === "changelog" && !selectedPost && (
          <div className="animate-fade-in space-y-6">
            <h2 className="text-2xl font-bold">📋 Changelog</h2>
            {changelogs.length === 0 && loaded ? (
              <Empty text="No changelogs yet." />
            ) : (
              <div className="space-y-4">
                {changelogs.map((p) => <LogCard key={p.id} post={p} onClick={() => openPost(p)} fmt={fmt} />)}
              </div>
            )}
          </div>
        )}
        ...
        {tab === "post" && selectedPost && (
          <article className="animate-fade-in max-w-3xl mx-auto">
            <button onClick={() => { setTab(selectedPost.type === "changelog" ? "changelog" : "blog"); setSelectedPost(null); }}
              className="text-accent text-sm hover:underline mb-6 block"
            >
              ← Back to {selectedPost.type === "changelog" ? "Changelog" : "Blog"}
            </button>
            <div className="mb-6">
            ...
            {selectedPost.tags && (selectedPost.tags as string[]).length > 0 && (
              <div className="flex gap-2 mb-6 flex-wrap">
                {(selectedPost.tags as string[]).map((t: string) => (
                  <span key={t} className="px-2 py-0.5 bg-bg-secondary text-text-muted rounded text-xs">#{t}</span>
                ))}
              </div>
            )}
            <div className="bg-bg-card border border-border rounded-xl p-8 text-text-secondary leading-relaxed whitespace-pre-wrap">
              {selectedPost.body}
            </div>
          </article>
        )}
...
function PostCard({ post, onClick, fmt }: { post: CmsPost; onClick: () => void; fmt: (d: string) => string }) {
  return (
    <button onClick={onClick} className="bg-bg-card border border-border rounded-xl p-6 text-left hover:border-accent/30 transition-all hover:shadow-lg w-full">
...
function LogCard({ post, onClick, fmt }: { post: CmsPost; onClick: () => void; fmt: (d: string) => string }) {
  return (
    <button onClick={onClick} className="bg-bg-card border border-border rounded-xl p-5 text-left hover:border-accent/30 transition-all w-full flex gap-4">
      <div className="flex-shrink-0 w-24 text-xs text-text-muted">{fmt(post.createdAt)}</div>
