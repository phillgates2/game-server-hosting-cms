...
async function loadServerAndTasks(idNum: number) {
  const [server] = await db
    ...
    .limit(1);
  if (!server) return { server: null, tasks: [] as RestartTask[] };
  const tasks = (await db
    ...
    .from(scheduledTasks)
    .where(and(eq(scheduledTasks.serverId, idNum), eq(scheduledTasks.taskType, "restart")))) as RestartTask[];
  return { server, tasks };
}

function dailyTask(tasks: RestartTask[]): { task: RestartTask; hour: number; minute: number } | null {
...
// GET — current daily-restart state for this server
export async function GET(
  ...
  try {
    ...
    return apiError(e, "Could not load the daily restart schedule", 500);
  }
}

// POST — { enabled, hour?, minute? }: create/update/disable the daily restart
export async function POST(
  ...
  try {
    ...
    if (!enabled) {
      // Disable (never delete) so switching the toggle back on is honest and
      // custom schedules elsewhere are untouched.
      if (daily) {
        await db
    ...
    return apiError(e, "Could not update the daily restart schedule", 500);
  }
}
