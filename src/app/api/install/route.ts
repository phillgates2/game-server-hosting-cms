...
export async function POST(req: NextRequest) {
  try {
    ...
    await logStep("schema", "running", "Creating database tables...");
    ...
    `);
    await logStep("schema", "done", "Database tables created successfully");
    ...
    await logStep("game_templates", "done", `${gameTemplates.length} game templates available. Install them from the Games panel.`);
    ...
    const forumCats = [
      ...
      { name: "Server Support", slug: "server-support", description: "Help with game server setup and configuration", sortOrder: 2 },
    ...
    const settingsData = [
      ...
      { key: "auto_clear_buffers", value: "true" },
